    ...
    public function edit(){
        ...
        if(IS_POST){
            ...
            if(strlen($uname) < 3 || strlen($uname) > 26){
                return $this->error('用户名3-26个字符之间.', tsurl('/admin/admin/edit',array('id'=>$id)));
            }
            $data = array();
            $data['uname'] = $uname;
            $data['status'] = $status;
            if($password){
                ...
                if($password != $password2){
                    return $this->error('两次输入密码不对', tsurl('/admin/admin/edit',array('id'=>$id)));
                }
                $data['salt'] = random(12,'all');
                $data['pwd'] = encrypt_password($password, $data['salt']);
            }else{
                if(empty($id)){
                    return $this->error('请输入登录密码',tsurl('/admin/admin/edit'));
                }
            }
            ...
            if($id){
                ...
                if($user && $user['id'] != $id){
                    return $this->error('用户名已经存在',tsurl('/admin/admin/edit', array('id'=>$id)));
                }
                $res = M('admin')->where(array('id'=>$id))->save($data);
            }else{
                $user = M('admin')->where(array('uname'=>$uname))->find();
                if($user){
                    return $this->error('用户名已经存在',tsurl('/admin/admin/edit'));
                }
                $res = M('admin')->add($data);
            }

            if($res){
                return $this->success('保存成功',tsurl('/admin/admin/index'));
            }else{
                return $this->error('编辑失败',tsurl('/admin/admin/edit',array('id'=>$id)));
            }
